...
//Mogo Posititions
const int mogoPos[9] = {350, 1600, 1660, 1747, 1870, 2100, 2310, 2580, 2790};
#define zeroM 350
	...
	while(!breaker){
		...
		if(vexRT[Btn5U] == 0){
			...
			while(!breaker && loopIsRunning){
				...
				armPosTarget = 0;
				while(SensorValue[liftDown] == 0 && !breaker){
				}
				wait1Msec(100);
				loopIsRunning = false;
			}
...
{
PID armController;
initPID(armController, 0.27, 0.0, 0.4);
...
{
PID driveController;
initPID(driveController, 0.23, 0.004, 0.03);
...
coneCount = 1;
while(getPot() <mogoPosTarget-150)
{}
wait1Msec(500);
