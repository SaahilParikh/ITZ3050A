...
const int mogoPos[9] = {350, 1660, 1660, 1640, 1820, 1973, 2403, 2770, 2870};
#define zeroM 350
...
//Arm Postitions
/////////////////////
const int armPos[9] = {-20, 400, 433, 413, 412, 393, 380, 350, 380};
#define zeroA 0
#define oneA 0
#define twoA 0
#define threeA 0
...
{
PID armController;
initPID(armController, 0.29, 0.0, 0.4);
//setMinSpeedPID(armController, -20);

while(true)
{

	int armPowerL = armPIDCalculation(armPosTarget, armController, -nMotorEncoder[liftLeft]);
	int armPowerR = armPIDCalculation(armPosTarget, armController, -nMotorEncoder[liftRight]);
...
PID turnController;
initPID(turnController, 0.15, 0.00, 0.00);
...
while(true)
{
	...
	int driveTurnPower = turnToHeadingCalucation(turnPosTarget, turnController);

	drive(drivePowerL, drivePowerR);
	writeDebugStreamLine("drivePid: Target: %d LeftPower: %f RightPower: %f leftEnc: %d rightEnc: %d", drivePosTarget, drivePowerL, drivePowerR, SensorValue[encLeft], SensorValue[encRight]);
	delay(10);
}
}
...
{
PID driveController;
initPID(driveController, 0.3, 0.004, 0.03);
...
while(true)
{
	int drivePowerL = drivePIDCalculation(turnPosTarget, driveController, driveLeftEnc);
	int drivePowerR = drivePIDCalculation(-turnPosTarget, driveController, -driveRightEnc);
...
motor[driveFrontRight] = 127;
wait1Msec(250);
motor[driveBackLeft] = 10;
motor[driveBackRight] = -10;
