...
void driveKeepHeading(int encoderCount, int gyroHeading = targetHeading, int addedTime = 0, int stationaryCount = driveStraightStationaryCountThresh, bool display = false)
{
	...
	PID turnController;
	initPID(turnController, 0.5, 0.00, 0.2);
	...
	while( ((time1[T1]-addedTime) < estimatedTime) && (statCount < stationaryCount))
	{
		...
		drivePowerR = drivePIDCalculation(encoderCount, driveController, driveRightEnc);
		drivePowerL = drivePIDCalculation(encoderCount, driveController, driveLeftEnc);

		if(abs(targetHeading-getHeading)>5)
		...
		if(	abs(targetHeading-getHeading) < 15 && (abs(drivePosTarget-getDriveEnc()) < 30 )
		{
			statCount++;
		}
		else
...
void drivePID (int encoderCount, int bias = 0, int addedTime = 0, int stationaryCount = driveStraightStationaryCountThresh, bool display = false, int targetHeadings = targetHeading)
{

	PID driveController;
	initPID(driveController, 1.2, 0.000, 0.1);
	PID turnController;
	initPID(turnController, 6.0, 0.0001, 300);
	...
	int drivePower = drivePIDCalculation(encoderCount, driveController, getDriveEnc());
	int turnPower = turnToHeadingCalucation(targetHeadings, turnController);


	while( ((time1[T1]-addedTime) < estimatedTime) && (statCount < stationaryCount))
	{

		drivePower = drivePIDCalculation(encoderCount, driveController, getDriveEnc());
		turnPower = turnToHeadingCalucation(targetHeadings, turnController);

		drive(drivePower-bias-turnPower, drivePower+bias-turnPower);

		if(display)
			writeDebugStreamLine("%d,      ,%d, target: %d, GetDriveend(): %f, driveTimeConst, %f ,   abs(getDriveEnc()-encoderCount): %d", statCount , (-(time1[T1]-addedTime) + estimatedTime), encoderCount, getDriveEnc(), driveTimeConst, (abs(encoderCount-getDriveEnc())) );

		if((abs(drivePosTarget-getDriveEnc()) < 30 )
		{
			statCount++;
		}
		else
...
void driveTurn (int targetHeadings, int addedTime = 0, float kPTurn = 0.35, int stationaryCount = turnStraightStationaryCountThresh, bool display = false)
{

		targetHeading = targetHeadings;

		PID turnController;
		initPID(turnController, kPTurn, 0.0001, 300);
		...
		while((time1[T2]-addedTime) < (estimatedTime) && statCount < stationaryCount)
		{
			...
			if(	abs(targetHeading-getHeading) < 20 )
			{
				statCount++;
			}
		 	else
...
void setDrive(int target, int addedTime = 0, int statC = driveStraightStationaryCountThresh)
{
		time1[T1] = 0;
		drivePosTarget = target;
		int estimatedTime = abs(driveTimeConst*(target-getDriveEnc());
		int statCount = 0;

		while((time1[T1]-addedTime) < estimatedTime && statCount < statC)
		{
		writeDebugStreamLine("%d,      ,%d, target: %d, GetDriveend(): %f, driveTimeConst, %f ,   abs(getDriveEnc()-target): %d", statCount , (-(time1[T1]-addedTime) + estimatedTime), target, getDriveEnc(), driveTimeConst, (abs(drivePosTarget-getDriveEnc())) );
...
void setTurn(int target, int addedTime = 0, int scont = turnStraightStationaryCountThresh)
{
		time1[T2] = 0;

		int estimatedTime = abs(turnTimeConst*(target-getHeading);
		...
		while((time1[T2]-addedTime) < (estimatedTime) && statCount < scont)
		{
